	...
	<main class="flex flex-wrap gap-3">
		<form action="" class="flex w-full items-center gap-4">
			<label for="" class="flex flex-col">
				<span>Book ID</span>
				<input v-model="queryMedia.book_id" type="search" class="input" list="idList" />
				<datalist id="idList">
		...
		<div v-for="dir in filteredMediaList" :key="dir.bookID">
			...
			<button v-for="(img, index) in dir.mediaList" :key="index" class="p-2" @click="showMediaDialog(dir.bookID, img)">
				<video
					...
					class="size-44" />
				<figure v-else class="h-full" v-lazy-load>
